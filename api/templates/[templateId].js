...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.log('🔍 [templates] 특정 템플릿 조회:', templateId);
    ...
    if (error) {
      console.error('❌ [templates] Supabase 오류:', error);
      return res.status(500).json({ error: 'Database error' });
    }

    if (!template) {
      console.log('❌ [templates] 템플릿을 찾을 수 없음:', templateId);
      return res.status(404).json({ error: 'Template not found' });
    }

    console.log('✅ [templates] 템플릿 조회 성공:', template.name);
    ...
    console.error('❌ [templates] 서버 오류:', error);
