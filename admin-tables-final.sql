-- designs 테이블과 완전히 분리된 관리자 페이지용 테이블 생성
...
-- 3. 관리자용 별도 함수 생성 (기존 함수와 충돌 방지)
CREATE OR REPLACE FUNCTION admin_update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
CREATE TRIGGER update_categories_updated_at 
    ...
    FOR EACH ROW EXECUTE FUNCTION admin_update_updated_at_column();
...
-- 7. 완료 메시지
SELECT 'Admin tables created successfully with separate function!' as status;
