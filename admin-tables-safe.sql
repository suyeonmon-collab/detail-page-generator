-- designs 테이블은 건드리지 않고 관리자 페이지용 테이블만 생성
...
-- 4. 관리자 테이블용 트리거만 생성 (designs 테이블은 건드리지 않음)
DO $$ 
BEGIN
    -- categories 테이블 트리거 생성
    IF NOT EXISTS (
...
-- 7. 완료 메시지
SELECT 'Admin tables created successfully without touching designs table!' as status;
