...
async function generateAIContent(data) {
  try {
    ...
    // OpenAI API 사용 (Claude는 나중에 전환 가능)
    ...
    if (!response.ok) {
      ...
      throw new Error(`AI API 오류: ${errorData.error?.message || 'API 호출 실패'}`);
    }
...
async function applyToFigmaTemplate(data, aiContent) {
  try {
    ...
    // 워터마크 포함 미리보기 URL (실제로는 Figma에서 생성)
